...
def getConfigZYX(start_x, end_x):
    ...
    return (g1, g2, f1, f2), matrix, side_surface, front_surface
...
def generateVolume(axes, start_x, end_x, resolution_density=800):
    ## CONFIGURATION
    [g1, g2, f1, f2], matrix, side_surf, front_surf = getConfigZXY(start_x, end_x)
...
#################### MAIN ####################################################
class NewIntegrationOrder3(MyThreeDScene):
...
#Initial integration steps #dz is now dz, dy is now dx
...
################################  MAIN LATEX  ############################################
        ...
        y_integration = MathTex(
            r"\int_{x=0}^{x=\sqrt{y}}", font_size = 36
        )
