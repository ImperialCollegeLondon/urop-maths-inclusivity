...
def generateVolume(axes, start_x, end_x, resolution_density=250):
    ## CONFIGURATION
    [g1, g2, f1, f2], matrix, top_surf, side_surf, front_surf = getConfigZXY(start_x, end_x)
...
#################### MAIN ####################################################
class NewIntegrationOrder4(MyThreeDScene):
...
#Curves
        ...
        curve2 = Line(axes.c2p(0,0,0),axes.c2p(0,1.5,0)).set_color(GOLD)
...
#Initial integration steps #dz is now dx, dy is now dz
        dz_coord_start = axes.c2p(0.5,0.5,0.25)
        dz_coord_end = axes.c2p(0.45,0.5,0.25)
        dz = Line(dz_coord_start,dz_coord_end,color=YELLOW,stroke_width=2.5)

        start_point = axes.c2p(0.25,0.5,0.25)
        end_point = axes.c2p(0.707106,0.5,0.25)
...
#Limit Labels
    ...
    # x limits
        ...
        x_b_line = DashedLine(
            start = axes.c2p(0,1,0),
            end = axes.c2p(1.5,1,0)
        ).set_color(BLUE)
